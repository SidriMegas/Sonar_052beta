...
export async function POST(req: Request) {
    try {
        ...
        // 🎬 APPEL YOUTUBE API (AJOUT DE contentDetails POUR LA DURÉE)
        if (!YT_KEY) {
        ...
        // --- 🛑 VÉRIFICATION DE LA DURÉE (MAX 10 MINUTES) ---
        const durationISO = item.contentDetails?.duration
        if (durationISO) {
            const totalSeconds = parseYouTubeDuration(durationISO)
            if (totalSeconds > 600) { // 600 secondes = 10 minutes
                return NextResponse.json(
                    { error: "Format invalide : La vidéo dépasse 10 minutes (mixes/albums interdits)." },
                    { status: 403 }
                )
            }
        }
        ...
        // 💎 CALCULER SCORE AVEC FORMULE UNIFORME
...
export async function GET() {
    return NextResponse.json({
        message: "✅ Robot de partage opérationnel (avec limite 10min)",
